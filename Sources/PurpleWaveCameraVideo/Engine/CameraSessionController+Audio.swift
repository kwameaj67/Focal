...
//  PurpleWaveCameraPhoto never pulls in microphone code — and therefore has no
...
extension CameraSessionController {
    ...
    ///
    /// - Throws: `PWCameraError.sessionConfigurationFailed` if there's no
    ///   microphone or the input can't be added.
    nonisolated func attachAudioInput() throws {
        guard let mic = AVCaptureDevice.default(for: .audio) else {
            throw PWCameraError.sessionConfigurationFailed("No microphone available")
        }
        let input = try AVCaptureDeviceInput(device: mic)
        guard session.canAddInput(input) else {
            throw PWCameraError.sessionConfigurationFailed("Cannot add audio input")
        }
