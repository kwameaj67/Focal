...
//  FocalVideo.
//
//  Core still names the microphone as a `FCPermission` case, and
...
//  part that matters: a host linking only FocalPhoto needs no
...
public enum FCPermission: Sendable {
    case camera
    case microphone
    case photoLibrary
    case location
...
public enum FCPermissionStatus: Sendable {
    case granted
...
public enum MediaPermissions {
    ...
    public static func cameraStatus() -> FCPermissionStatus {
    ...
    public static func ensurePhotoPermissions() async -> FCPermission? {
    ...
    /// `package` so the microphone accessors in FocalVideo can
    /// reuse the same normalization. Not public: hosts get the mapped
    /// `FCPermissionStatus` values, never the AVFoundation ones.
    package static func map(_ status: AVAuthorizationStatus) -> FCPermissionStatus {
        switch status {
    ...
    private static func map(_ status: PHAuthorizationStatus) -> FCPermissionStatus {
        switch status {
        case .authorized:    return .granted
