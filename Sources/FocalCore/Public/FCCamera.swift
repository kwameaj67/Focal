...
//  `makePhotoCapture` lives in FocalPhoto and `makeVideoRecorder` in
//  FocalVideo. A host that links only one of them sees only that
...
///
/// SwiftUI consumers don't need this — present `FCCameraScreen` or
/// `FCVideoScreen` directly.
public enum FCCamera {}
