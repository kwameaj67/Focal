...
package extension Color {
    ...
    static let fcSettingsSheet = Color(red: 0.11, green: 0.11, blue: 0.12)
}
...
package struct AspectRatioSelector: View {
    ...
    @Binding private var selection: FCAspectRatio

    package init(selection: Binding<FCAspectRatio>) {
        self._selection = selection
    }

    package var body: some View {
        HStack(spacing: 12) {
            ForEach(FCAspectRatio.allCases, id: \.self) { ratio in
...
#Preview("Settings sheet — sub-overlay with back") {
    @Previewable @State var ratio: FCAspectRatio = .sixteenNine
    ZStack(alignment: .bottom) {
        Color.black
...
#Preview("Aspect selector — each option") {
    @Previewable @State var ratio: FCAspectRatio = .fourThree
    ZStack {
        Color.black
        VStack(spacing: 24) {
            AspectRatioSelector(selection: $ratio)
