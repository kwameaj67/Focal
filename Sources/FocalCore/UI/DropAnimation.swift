...
/// Places (and shrinks) the flying capture image along `FCDropPath`.
///
...
package struct FCDropEffect: GeometryEffect {
    ...
    package func effectValue(size: CGSize) -> ProjectionTransform {
        let point = FCDropPath.point(progress: progress, start: start, end: end)
        let scale = FCDropPath.scale(progress: progress)
        ...
        GeometryReader { proxy in
            if let image, let anchor {
                ...
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
                    .frame(width: FCDropPath.baseSize, height: FCDropPath.baseSize)
                    .clipShape(RoundedRectangle(cornerRadius: 8))
                    .modifier(FCDropEffect(progress: progress, start: start, end: end))
            }
        }
