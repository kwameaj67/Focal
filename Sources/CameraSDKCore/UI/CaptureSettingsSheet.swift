...
package extension Color {
    ...
    static let csSettingsSheet = Color(red: 0.11, green: 0.11, blue: 0.12)
}
...
package struct AspectRatioSelector: View {
    ...
    @Binding private var selection: CSAspectRatio

    package init(selection: Binding<CSAspectRatio>) {
        self._selection = selection
    }

    package var body: some View {
        HStack(spacing: 12) {
            ForEach(CSAspectRatio.allCases, id: \.self) { ratio in
...
#Preview("Settings sheet — sub-overlay with back") {
    @Previewable @State var ratio: CSAspectRatio = .sixteenNine
    ZStack(alignment: .bottom) {
        Color.black
...
#Preview("Aspect selector — each option") {
    @Previewable @State var ratio: CSAspectRatio = .fourThree
    ZStack {
        Color.black
        VStack(spacing: 24) {
            AspectRatioSelector(selection: $ratio)
