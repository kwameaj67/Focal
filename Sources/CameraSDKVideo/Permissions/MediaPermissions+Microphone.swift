...
//  only CameraSDKPhoto contains no microphone code path, so it needs no
...
public extension MediaPermissions {
    ...
    static func microphoneStatus() -> CSPermissionStatus {
    ...
    static func ensureVideoPermissions() async -> CSPermission? {
