...
public struct CSVideoScreen: View {

    // MARK: Inputs

    private let config: CSVideoConfig
    private let handlers: CSVideoHandlers
    ...
    @State private var aspectRatio: CSAspectRatio = .sixteenNine
    ...
    public init(config: CSVideoConfig, handlers: CSVideoHandlers) {
        self.config = config
    ...
    private var previewCard: some View {
        ZStack {
            ...
            if config.landscapeOnly && !orientation.orientation.csIsLandscape {
    ...
    private var orientationAllowsRecording: Bool {
        !config.landscapeOnly || orientation.orientation.csIsLandscape
    }
    ...
    private func handleRecording(_ result: Result<VideoRecordEngine.RecordedVideo, Error>) {
        ...
        case .success(let video):
            ...
            Task {
                ...
                handlers.onRecord(CSVideoResult(
                    fileURL: video.url,
...
#Preview("Video screen — full chrome") {
    CSVideoScreen(
        config: CSVideoConfig(
            categories: [
                CSCategory(id: "DRIVING", title: "Driving"),
                CSCategory(id: "FUNCTIONAL", title: "Functional"),
                CSCategory(id: "ENGINE", title: "Engine")
            ],
            startingCategoryID: "DRIVING",
            maxDuration: 180,
            overlayLabel: "MO6759"
        ),
        handlers: CSVideoHandlers(onRecord: { _ in })
    )
}
...
#Preview("Video screen — torch, gallery and ultra-wide off") {
    ...
    CSVideoScreen(
        config: CSVideoConfig(
            categories: [CSCategory("Driving"), CSCategory("Functional")],
            allowsTorch: false,
