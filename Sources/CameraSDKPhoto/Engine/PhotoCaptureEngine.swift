...
final class PhotoCaptureEngine: NSObject, ObservableObject {
    ...
    @Published var flashMode: CSFlashMode
    ...
    private let controller = CameraSessionController(
        queueLabel: "com.camera-sdk.camera.photo.session"
    )
    ...
    init(defaultFlashMode: CSFlashMode) {
    ...
    /// - Throws: `CSCameraError.sessionConfigurationFailed` if no camera is
    ...
    nonisolated private func configureSessionOnQueue() throws {
        ...
        photoOutput.connection(with: .video)?.csSetRotationAngle(90)
    }
    ...
    /// Requests a capture/preview frame rate in fps (clamped to what the format
    /// supports).
    ...
    /// - Throws: `CSCameraError.photoCaptureFailed` on any failure.
    func capturePhoto(orientation: UIDeviceOrientation) async throws -> CapturedPhoto {
        guard !isProcessing else {
            throw CSCameraError.photoCaptureFailed("Capture already in progress")
        }
        isProcessing = true
        ...
        let captureAngle = orientation.csCaptureRotationAngle
        ...
        return try await withCheckedThrowingContinuation { cont in
            ...
            controller.sessionQueue.async { [weak self] in
                ...
                guard self.session.isRunning,
                    ...
                    Task { @MainActor in
                        self.finishCapture(
                            .failure(CSCameraError.photoCaptureFailed("Session not ready"))
                        )
                    }
                    return
                }
                ...
                connection.csSetRotationAngle(captureAngle)
    ...
    /// mode, a "camerasdk_captured" EXIF marker, and max resolution). Runs on
    ...
    nonisolated private func makeSettings(flash: AVCaptureDevice.FlashMode) -> AVCapturePhotoSettings {
        ...
        settings.metadata = [
            kCGImagePropertyExifDictionary as String: [
                kCGImagePropertyExifUserComment as String: "camerasdk_captured"
            ]
        ]
...
extension PhotoCaptureEngine: AVCapturePhotoCaptureDelegate {
    ...
    nonisolated func photoOutput(
        ...
        if let error {
            outcome = .failure(CSCameraError.photoCaptureFailed(error.localizedDescription))
        } else if let data = photo.fileDataRepresentation(),
                  let image = UIImage(data: data) {
            outcome = .success(CapturedPhoto(data: data, image: image, metadata: photo.metadata))
        } else {
            outcome = .failure(CSCameraError.photoCaptureFailed("No image data"))
        }
