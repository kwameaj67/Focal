...
public protocol CSPhotoCaptureDelegate: AnyObject {
    ...
    func photoCapture(didCapture result: CSPhotoResult)

    /// Called when the user leaves the screen (Done or Cancel).
    func photoCapture(didFinish reason: CSFinishReason)

    /// Called when a recoverable error occurs (e.g. a single failed capture).
    func photoCapture(didFail error: CSCameraError)
    ...
    func photoCapture(didImport results: [CSPhotoResult])
}

public extension CSPhotoCaptureDelegate {
    func photoCapture(didFinish reason: CSFinishReason) {}
    func photoCapture(didFail error: CSCameraError) {}
    func photoCapture(didImport results: [CSPhotoResult]) {}
}

/// Closure-based equivalent of `CSPhotoCaptureDelegate`, convenient for SwiftUI
/// and one-off presentations.
public struct CSPhotoHandlers {
    public var onCapture: (CSPhotoResult) -> Void
    ...
    public var onImport: ([CSPhotoResult]) -> Void
    public var onFinish: (CSFinishReason) -> Void
    public var onError: (CSCameraError) -> Void

    public init(
        onCapture: @escaping (CSPhotoResult) -> Void,
        onImport: @escaping ([CSPhotoResult]) -> Void = { _ in },
