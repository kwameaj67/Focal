...
package final class CameraSessionController {
    ...
    ///   `"com.purplewave.camera.photo.session"`. Distinct labels keep the two
    ...
    ///
    /// - Throws: `PWCameraError.sessionConfigurationFailed` if the device is
    ...
    package nonisolated func attachVideoInput(_ deviceType: AVCaptureDevice.DeviceType) throws {
        guard let target = AVCaptureDevice.default(deviceType, for: .video, position: .back) else {
            throw PWCameraError.sessionConfigurationFailed("No back camera available")
        }
        ...
        guard session.canAddInput(input) else {
            ...
            throw PWCameraError.sessionConfigurationFailed("Cannot add camera input")
        }
    ...
    /// Must be called on `sessionQueue`, inside a configuration transaction.
    ///
    /// - Throws: `PWCameraError.sessionConfigurationFailed` if it can't be added.
    package nonisolated func attachOutput(_ output: AVCaptureOutput) throws {
        guard session.canAddOutput(output) else {
            throw PWCameraError.sessionConfigurationFailed("Cannot add capture output")
        }
