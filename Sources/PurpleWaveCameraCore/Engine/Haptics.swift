...
package extension HapticsManager {
    ...
    func tap() {
        ...
        play(time: 0, intensity: 0.93, sharpness: 0.04)    }

    /// Subtle tick for selection changes: tabs, pickers, day pills, grids.
    func selection() {
        guard supportsHaptics else { return }
        play(time: 0, intensity: 0.4, sharpness: 0.35)
    }

    /// Firm thud for significant actions: starting/stopping a session.
    func impact() {
        guard supportsHaptics else { return }
        play(time: 0, intensity: 0.9, sharpness: 0.6)
    }

    /// Rising double tick for a completed action: session saved, block added.
    func success() {
        guard supportsHaptics else { return }
        play(time: 0, intensity: 0.6, sharpness: 0.4)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.12) { [weak self] in
            self?.play(time: 0, intensity: 1.0, sharpness: 0.7)
        }
    }
}
