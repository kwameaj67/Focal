...
//  reimplementation of FieldTool's `CameraSelectionCollectionViewController`.
//
...
package struct CategoryScrollBar: View {
    ...
    package let categories: [PWCategory]
    @Binding package var selected: PWCategory?
    ...
    private func pill(for category: PWCategory) -> some View {
    ...
    private func select(_ category: PWCategory, proxy: ScrollViewProxy) {
    ...
    private func center(on category: PWCategory?, proxy: ScrollViewProxy, animated: Bool) {
...
#Preview("Category bar") {
    @Previewable @State var selected: PWCategory? = PWCategory("Profile")
    ZStack {
        Color.black
        CategoryScrollBar(
            categories: [
...
#Preview("Category bar — single option") {
    ...
    ZStack {
        Color.black
        CategoryScrollBar(categories: [PWCategory("Profile")], selected: $selected)
    }
    .ignoresSafeArea()
}
