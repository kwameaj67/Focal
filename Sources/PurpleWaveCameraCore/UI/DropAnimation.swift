...
/// Places (and shrinks) the flying capture image along `PWDropPath`.
///
...
package struct PWDropEffect: GeometryEffect {
    ...
    package func effectValue(size: CGSize) -> ProjectionTransform {
        let point = PWDropPath.point(progress: progress, start: start, end: end)
        let scale = PWDropPath.scale(progress: progress)
        ...
        GeometryReader { proxy in
            if let image, let anchor {
                ...
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
                    .frame(width: PWDropPath.baseSize, height: PWDropPath.baseSize)
                    .clipShape(RoundedRectangle(cornerRadius: 8))
                    .modifier(PWDropEffect(progress: progress, start: start, end: end))
            }
        }
