...
final class PhotoCaptureEngine: NSObject, ObservableObject {
    ...
    @Published var flashMode: FCFlashMode
    ...
    private let controller = CameraSessionController(
        queueLabel: "com.focal.camera.photo.session"
    )
    ...
    init(defaultFlashMode: FCFlashMode) {
    ...
    /// - Throws: `FCCameraError.sessionConfigurationFailed` if no camera is
    ...
    nonisolated private func configureSessionOnQueue() throws {
        ...
        photoOutput.connection(with: .video)?.fcSetRotationAngle(90)
    }
    ...
    /// - Throws: `FCCameraError.photoCaptureFailed` on any failure.
    func capturePhoto(orientation: UIDeviceOrientation) async throws -> CapturedPhoto {
        guard !isProcessing else {
            throw FCCameraError.photoCaptureFailed("Capture already in progress")
        }
        isProcessing = true
        ...
        let captureAngle = orientation.fcCaptureRotationAngle
        ...
        return try await withCheckedThrowingContinuation { cont in
            ...
            controller.sessionQueue.async { [weak self] in
                ...
                guard self.session.isRunning,
                    ...
                    Task { @MainActor in
                        self.finishCapture(
                            .failure(FCCameraError.photoCaptureFailed("Session not ready"))
                        )
                    }
                    return
                }
                ...
                connection.fcSetRotationAngle(captureAngle)
    ...
    /// mode, a "focal_captured" EXIF marker, and max resolution). Runs on
    ...
    nonisolated private func makeSettings(flash: AVCaptureDevice.FlashMode) -> AVCapturePhotoSettings {
        ...
        settings.metadata = [
            kCGImagePropertyExifDictionary as String: [
                kCGImagePropertyExifUserComment as String: "focal_captured"
            ]
        ]
...
extension PhotoCaptureEngine: AVCapturePhotoCaptureDelegate {
    ...
    nonisolated func photoOutput(
        ...
        if let error {
            outcome = .failure(FCCameraError.photoCaptureFailed(error.localizedDescription))
        } else if let data = photo.fileDataRepresentation(),
                  let image = UIImage(data: data) {
            outcome = .success(CapturedPhoto(data: data, image: image, metadata: photo.metadata))
        } else {
            outcome = .failure(FCCameraError.photoCaptureFailed("No image data"))
        }
