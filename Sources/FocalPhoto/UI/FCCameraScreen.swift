...
public struct FCCameraScreen: View {

    // MARK: Inputs

    private let config: FCCameraConfig
    private let handlers: FCPhotoHandlers
    ...
    @State private var aspectRatio: FCAspectRatio = .fourThree
    ...
    public init(config: FCCameraConfig, handlers: FCPhotoHandlers) {
        self.config = config
    ...
    private var infoTip: some View {
        InfoTip {
            VStack(alignment: .leading, spacing: 4) {
                Text(orientation.orientation.fcIsLandscape ? "4:3 Landscape" : "3:4 Portrait")
    ...
    private var settingsSheet: some View {
        ...
        case .flash:
            CaptureSettingsChrome(title: "Flash", onBack: settingsBack) {
                DiscreteWheel(
                    values: [FCFlashMode.auto, .on, .off],
                    selection: Binding(
    ...
    private func performCapture() {
        Task {
            do {
                ...
                handlers.onCapture(FCPhotoResult(
                    imageData: data,
                    image: image,
    ...
    FCCameraScreen(
        config: FCCameraConfig(
            categories: [
                ...
                FCCategory(id: "WALKAROUND", title: "Walk Around")
            ],
            startingCategoryID: "AMKT",
            overlayLabel: "MO6759"
        ),
        handlers: FCPhotoHandlers(onCapture: { _ in })
    )
}
...
#Preview("Photo screen — gallery and ultra-wide off") {
    ...
    FCCameraScreen(
        config: FCCameraConfig(
            categories: [FCCategory("Profile"), FCCategory("Sticker")],
            allowsGallery: false,
